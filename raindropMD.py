...
def get_templates_folder(base_dir: Path) -> Path:
    """
    Get the templates folder inside the base directory.
    Exits the program if the templates folder does not exist.
    """
    templates_dir = base_dir / "templates"
    if not templates_dir.exists() or not templates_dir.is_dir():
        console.print(Panel(f"[red]No templates/ folder found in {base_dir}![/red]", title="[red]Error[/red]", style="red"))
        sys.exit(1)
...
def search(bookmark_dir, query):
    ...
    if not templates:
        console.print(Panel(f"[red]No templates found in {templates_dir}![/red]", title="[red]Error[/red]", style="red"))
        sys.exit(1)
    fuzzy_search_bookmarks_dir(Path(bookmark_dir), query, templates_dir=templates_dir)
